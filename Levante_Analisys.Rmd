...
```{r}
# 1. AGREGACIÓN A ESCALA SEMESTRAL (separada para evitar sesgo por datos faltantes)
# El viento se agrega desde la serie COMPLETA, independientemente de la disponibilidad
# de datos de reclutamiento. De lo contrario, semestres con más meses sin reclutamiento
# tendrían artificialmente menos días de viento sumados.

# --- Viento: desde serie_levante completa ---
viento_semestral <- serie_levante %>%
  mutate(
    semestre = case_when(
...
# --- Reclutamiento: desde datos_unidos (sin filtrar por viento) ---
reclu_semestral <- datos_unidos %>%
  mutate(
    semestre = case_when(
      mes %in% c("Jan", "Feb", "Mar", "Apr", "May", "Jun") ~ 1,
      mes %in% c("Jul", "Aug", "Sep", "Oct", "Nov", "Dec") ~ 2
...
# --- Join: solo semestres con ambas variables disponibles ---
datos_semestral <- inner_join(
  viento_semestral, reclu_semestral,
  by = c("año", "semestre", "año_semestre")
) %>%
  filter(n_obs_validas > 0)
```
...
```{r}
# Correlación por semestre: Pearson y Spearman con p-valores
# Se reportan ambos métodos dado que la velocidad del viento no sigue distribución normal
# (test Jarque-Bera, p < 2.2e-16). Spearman es más robusto en ese caso.
cor_por_semestre <- datos_semestral %>%
  group_by(semestre) %>%
  summarise(
    r_pearson  = cor(dias_viento_semestre, reclutamiento_promedio, use = "complete.obs"),
    p_pearson  = cor.test(dias_viento_semestre, reclutamiento_promedio,
                          method = "pearson")$p.value,
    r_spearman = cor(dias_viento_semestre, reclutamiento_promedio,
                     method = "spearman", use = "complete.obs"),
    p_spearman = cor.test(dias_viento_semestre, reclutamiento_promedio,
...
kableExtra::kbl(
  cor_por_semestre,
  col.names = c("Semestre", "r Pearson", "p Pearson", "r Spearman", "p Spearman", "n"),
  caption = "Correlaciones Pearson y Spearman entre días de Levante y reclutamiento promedio por semestre. Se reportan ambos métodos por no-normalidad del viento."
) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
kableExtra::kbl(
  comparacion,
  caption = "Comparación de modelos: R² ajustado, AIC y p-valor del test F global. El modelo con interacción se selecciona por menor AIC, aunque la interacción tiene p = 0.075 (a = 0.10)."
) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
Se verifican los supuestos del modelo con interacción (normalidad de residuos, homoscedasticidad y autocorrelación temporal). Con n = 20 semestres, la potencia es limitada, por lo que se adopta a = 0.10 como umbral de significación.

```{r model-diagnostics-table}
sw_test <- shapiro.test(residuals(modelo_interaccion_sem))
bp_test <- lmtest::bptest(modelo_interaccion_sem)
dw_test <- lmtest::dwtest(modelo_interaccion_sem)
...
diag_tabla <- data.frame(
  ...
  Estadístico = round(c(sw_test$statistic,
                        bp_test$statistic,
                        dw_test$statistic), 3),
  p_valor = signif(c(sw_test$p.value,
                     bp_test$p.value,
                     dw_test$p.value), 3),
  Conclusión = c(
    ...
    ifelse(bp_test$p.value > 0.05,
           "Varianza homogénea (p > 0.05)",
           "Heterocedasticidad (p < 0.05)"),
    ifelse(dw_test$p.value > 0.05,
           "Sin autocorrelación (p > 0.05)",
...
kableExtra::kbl(
  ...
  caption = "Tests de diagnóstico del modelo con interacción viento × semestre."
) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```

```{r model-diagnostics-plots, fig.height=6, fig.width=8, out.width="95%", fig.cap="Gráficos de diagnóstico del modelo seleccionado: residuos vs. ajustados, Q-Q, escala-localización e influencia."}
par(mfrow = c(2, 2))
plot(modelo_interaccion_sem)
par(mfrow = c(1, 1))
```

```{r cook-distance, out.width="70%", fig.cap="Distancia de Cook por semestre. La línea roja indica el umbral 4/n. Observaciones por encima pueden influir excesivamente en los coeficientes."}
cook_d  <- cooks.distance(modelo_interaccion_sem)
umbral  <- 4 / nrow(datos_semestral)

cook_df <- data.frame(
  obs          = seq_along(cook_d),
  cook         = cook_d,
  año_semestre = datos_semestral$año_semestre,
  influyente   = cook_d > umbral
)

ggplot(cook_df, aes(x = obs, y = cook, fill = influyente)) +
  geom_col(alpha = 0.8) +
  geom_hline(yintercept = umbral, linetype = "dashed", color = "red", linewidth = 0.8) +
  geom_text(data = subset(cook_df, influyente),
            aes(label = año_semestre), vjust = -0.5, size = 2.8, color = "red") +
  scale_fill_manual(values = c("FALSE" = "steelblue", "TRUE" = "tomato"),
                    labels = c("Normal", "Influyente"),
                    name = "") +
  labs(x = "Observación (semestre)",
       y = "Distancia de Cook",
       subtitle = paste0("Umbral = 4/n = ", round(umbral, 3),
                         " | n = ", nrow(datos_semestral), " semestres")) +
  theme_minimal() +
  theme(legend.position = "bottom")
```
...
```{r resultlm, fig.pos="ht!", fig.cap="Cada punto representa un semestre. Líneas y bandas de IC al 95% proceden del modelo de interacción conjunto (no de modelos separados por semestre). n total indicado en el subtítulo."}
# Rango de viento para predicciones
rango_viento <- seq(min(datos_semestral$dias_viento_semestre),
                    max(datos_semestral$dias_viento_semestre),
                    length.out = 200)

# Data frame para predicciones del modelo de interacción conjunto
nuevo_df <- expand.grid(
  dias_viento_semestre = rango_viento,
  semestre = factor(c(1, 2))
)

# ICs correctos: desde el modelo de interacción conjunto (NO geom_smooth separados)
pred_ic <- predict(modelo_interaccion_sem,
                   newdata = nuevo_df,
                   interval = "confidence",
                   level = 0.95)
nuevo_df <- cbind(nuevo_df, as.data.frame(pred_ic))

# Extraer coeficientes para las anotaciones (recalculados desde el modelo)
coef_m <- coef(modelo_interaccion_sem)
int_s1 <- round(coef_m["(Intercept)"], 1)
sl_s1  <- round(coef_m["dias_viento_semestre"], 2)
int_s2 <- round(coef_m["(Intercept)"] + coef_m["factor(semestre)2"], 1)
sl_s2  <- round(coef_m["dias_viento_semestre"] +
                  coef_m["dias_viento_semestre:factor(semestre)2"], 2)

r2_adj <- round(summary(modelo_interaccion_sem)$adj.r.squared, 2)
p_int  <- round(coef(summary(modelo_interaccion_sem))["dias_viento_semestre:factor(semestre)2", "Pr(>|t|)"], 3)

p_final <- ggplot() +
  # Bandas de IC al 95% del modelo conjunto
  geom_ribbon(data = filter(nuevo_df, semestre == "1"),
              aes(x = dias_viento_semestre, ymin = lwr, ymax = upr),
              fill = "darkred", alpha = 0.15) +
  geom_ribbon(data = filter(nuevo_df, semestre == "2"),
              aes(x = dias_viento_semestre, ymin = lwr, ymax = upr),
              fill = "steelblue", alpha = 0.15) +
  # Líneas de regresión
  geom_line(data = nuevo_df,
            aes(x = dias_viento_semestre, y = fit,
                color = semestre),
            linewidth = 1.2) +
  # Puntos observados
  geom_point(data = datos_semestral,
             ...
             size = 2.5, alpha = 0.85) +
  # Etiquetas de ecuación (desde coeficientes reales del modelo)
  annotate("text", x = max(rango_viento) * 0.55, y = max(datos_semestral$reclutamiento_promedio) * 0.35,
           label = paste0("Semestre 1 (Ene-Jun):\nReclut = ", int_s1,
                          ifelse(sl_s1 >= 0, " + ", " "), sl_s1, "×Viento\n(Efecto NEGATIVO)"),
           color = "darkred", size = 3.3, hjust = 0, fontface = "bold") +
  annotate("text", x = max(rango_viento) * 0.20, y = max(datos_semestral$reclutamiento_promedio) * 0.80,
           label = paste0("Semestre 2 (Jul-Dic):\nReclut = ", int_s2,
                          ifelse(sl_s2 >= 0, " + ", " "), sl_s2, "×Viento\n(Efecto POSITIVO)"),
           color = "steelblue", size = 3.3, hjust = 0, fontface = "bold") +
  ...
  labs(
    subtitle = paste0("R² adj = ", r2_adj,
                      " | n = ", nrow(datos_semestral), " semestres",
                      " | p interacción = ", p_int,
                      " | Banda: IC 95% del modelo conjunto"),
    x = "Días con viento de Levante por semestre",
    y = "Reclutamiento promedio (d15)",
...
### Relacion con Recruirt del modelo

```{r}

recrumod <- readxl::read_xlsx("~/IEO/SA_Donax_trunculus/outputs/tabla_resumen.xlsx")
head(recrumod)

levante <- read_csv("levante_componentes.csv")
...
# Reclutamiento anual (log)
recr_df <- recrumod %>%
  filter(Variable == "Recruitment",
         Scenario == "s3") %>%
  group_by(Year) %>%
  summarise(recruitment = mean(Media, na.rm = TRUE)) %>%
  mutate(log_recruit = log(recruitment))

# Viento anual
levante_df <- levante %>%
  mutate(year = year(fecha_quincena)) %>%
  group_by(year) %>%
  summarise(mean_vel = mean(velocidad_promedio, na.rm = TRUE))

# Unir
merged_df <- inner_join(recr_df, levante_df, by = c("Year" = "year"))


```

```{r}
scale_factor <- max(merged_df$log_recruit, na.rm = TRUE) / max(merged_df$mean_vel, na.rm = TRUE)

trendplot <- ggplot(merged_df, aes(x = Year)) +
  geom_line(aes(y = log_recruit, color = "Log Recruitment"), size = 1.1) +
  geom_line(aes(y = mean_vel * scale_factor, color = "Mean wind velocity"), size = 1.1, linetype = "dashed") +
  scale_y_continuous(
    name = "Log Recruitment",
    sec.axis = sec_axis(~ . / scale_factor, name = "Wind velocity (m/s)")
  ) +
  scale_color_manual(values = c("Log Recruitment" = "blue",
                                "Mean wind velocity" = "red")) +
  xlab("") +
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 90, vjust = 0.5, 
...
```{r}
ts_recr <- ts(merged_df$log_recruit, start = min(merged_df$Year))
ts_wind <- ts(merged_df$mean_vel, start = min(merged_df$Year))
n_total <- length(ts_recr)

# Obtener resultados de ccf (sin plot)
ccf_res <- ccf(ts_wind, ts_recr, lag.max = 5, plot = FALSE)

# Calcular n de pares efectivos por lag (n_total - |lag|)
ccf_df <- data.frame(
  lag   = as.numeric(ccf_res$lag),
  acf   = as.numeric(ccf_res$acf)
) %>%
  mutate(n_pares = n_total - abs(lag))

# Graficar con ggplot
# Nota: con n_total = 10-12 años, el lag 5 tiene solo ~5-7 pares → resultados inestables
gg_ccf <- ggplot(ccf_df, aes(x = lag, y = acf)) +
  geom_col(fill = "steelblue", alpha = 0.8) +
  geom_hline(yintercept = 0, color = "black") +
  geom_hline(yintercept = c(2, -2) / sqrt(n_total),
             linetype = "dashed", color = "red") +
  geom_text(aes(label = paste0("n=", n_pares)),
            vjust = ifelse(ccf_df$acf >= 0, -0.4, 1.2),
            size = 2.5, color = "gray30") +
  labs(title = "",
       subtitle = paste0("Serie de ", n_total,
                         " años. Líneas rojas = IC 95% asintótico (±2/√n_total).",
                         "\nEtiquetas indican n de pares efectivos por lag."),
       x = "Desfase (años)",
...
merged_df <- merged_df %>%
  arrange(Year) %>%
  mutate(wind_lag1 = dplyr::lag(mean_vel, 1),
         wind_lag2 = dplyr::lag(mean_vel, 2))
...
```{r}
# Lag 1
cor_lag1 <- cor.test(merged_df$log_recruit, merged_df$wind_lag1, use = "complete.obs")
cor_lag1

# Lag 2
cor_lag2 <- cor.test(merged_df$log_recruit, merged_df$wind_lag2, use = "complete.obs")
cor_lag2

```
```{r}
# Scatter con lag1
p1 <- ggplot(merged_df, aes(x = wind_lag1, y = log_recruit)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  theme_bw() +
  labs(x = "Wind velocity (lag 1 year)", y = "Log Recruitment",
       title = paste0("Lag 1: r = ", round(cor_lag1$estimate, 2),
                      ", p = ", signif(cor_lag1$p.value, 3)))

# Scatter con lag2
p2 <- ggplot(merged_df, aes(x = wind_lag2, y = log_recruit)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  theme_bw() +
  labs(x = "Wind velocity (lag 2 years)", y = "Log Recruitment",
       title = paste0("Lag 2: r = ", round(cor_lag2$estimate, 2),
                      ", p = ", signif(cor_lag2$p.value, 3)))
```
